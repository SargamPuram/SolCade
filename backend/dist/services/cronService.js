...
const calculateNextCronTime = () => {
    ...
    const nextMark = Math.floor(currentMinutes / CRON_INTERVAL_MINUTES) * CRON_INTERVAL_MINUTES +
...
const handleGameCron = async (gameObjectId, gameId, potService) => {
    ...
    try {
        ...
        const leaderboard = await Gameplay.find({
            ...
            .lean();
        result = leaderboard
            .map((entry) => entry.userId?.publicKey)
            .filter(Boolean);
        ...
        while (retries-- > 0 && !confirmed) {
            ...
            try {
                ...
                if (statusKey.toLowerCase() === "ended") {
                    confirmed = true;
                    console.log(`✅ Pot #${potNumber} confirmed as ended on-chain for ${gameId}`);
                }
                else {
                    console.log(`⏳ Waiting for pot #${potNumber} to be fully closed on-chain... (status: ${statusKey})`);
                }
            }
            catch (fetchErr) {
                console.log(`⏳ Retrying pot status check (${retries} retries left)...`);
            }
        }
        if (!confirmed) {
            console.error(`❌ Pot #${potNumber} not confirmed as closed on-chain for ${gameId}`);
            return;
        }
    }
    catch (err) {
...
export const initializeCronJobs = (games, potService) => {
    ...
    cron.schedule(CRON_EXPRESSION, async () => {
        ...
        const results = await Promise.allSettled(games.map((game) => handleGameCron(game.objectId, game.gameId, potService)));
        // Log results
        results.forEach((result, index) => {
            const game = games[index];
            if (result.status === "rejected") {
                console.error(`❌ Cron job error for ${game.gameId}:`, result.reason?.message || result.reason);
            }
            else {
