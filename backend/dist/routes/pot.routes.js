...
router.get("/:gameId/:potPublicKey", async (req, res) => {
    try {
        ...
        try {
            ...
            res.json({
                ...
                balanceSol: potAccount.totalLamports.toNumber() / anchor.web3.LAMPORTS_PER_SOL,
            });
        }
        catch (err) {
...
router.post("/distribute-winners", async (req, res) => {
    try {
        ...
        if (!gameId ||
            ...
            return res.status(400).json({
                error: "gameId, potPublicKey, and winners array with exactly 5 public keys are required",
            });
        }
        ...
        try {
            ...
            const [potPDA, _] = await PublicKey.findProgramAddress([Buffer.from("pot"), potPubkey.toBuffer()], program.programId);
        ...
        const errorResponse = { error: "Failed to distribute winnings" };
        if (error.message) {
...
router.post("/create-entry-fee-transaction", async (req, res) => {
    try {
...
router.post("/verify-payment", async (req, res) => {
    try {
        ...
        if (!gameId || !potPublicKey || !signature || !playerPublicKey) {
            return res.status(400).json({
                error: "gameId, potPublicKey, signature, and playerPublicKey are required",
            });
        }
        ...
        const programIndex = tx.transaction.message.accountKeys.findIndex((key) => key.equals(program.programId));
        if (programIndex === -1) {
            return res
                .status(400)
        ...
        const playerIndex = tx.transaction.message.accountKeys.findIndex((key) => key.equals(playerPubkey));
        if (playerIndex === -1) {
            return res
                .status(400)
...
router.post("/pay-entry-fee", async (req, res) => {
    try {
